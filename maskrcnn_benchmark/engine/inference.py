...
def custom_sgg_post_precessing(predictions):
    ...
    for idx, boxlist in enumerate(predictions):
        ...
        sortedid, id2sorted = get_sorted_bbox_mapping(boxlist.get_field('scores').tolist())
        ...
        for i in sortedid:
            ...
            bbox_labels.append(boxlist.get_field('labels')[i].item())
            bbox_scores.append(boxlist.get_field('scores')[i].item())

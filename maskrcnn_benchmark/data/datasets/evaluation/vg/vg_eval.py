...
def do_vg_evaluation(
    ...
    if "bbox" in iou_types:
        ...
        for image_id, prediction in enumerate(predictions):
            ...
            score = prediction.get_field('scores').detach().cpu().numpy() # (#objs,)
            label = prediction.get_field('labels').detach().cpu().numpy() # (#objs,)
...
def save_output(output_folder, groundtruths, predictions, dataset):
    if output_folder:
        ...
        for image_id, (groundtruth, prediction) in enumerate(zip(groundtruths, predictions)):
            ...
            prediction = [
                [b[0], b[1], b[2], b[3], dataset.categories[l]] # xyxy, str
                for b, l in zip(prediction.bbox.tolist(), prediction.get_field('labels').tolist())
                ]
            visual_info.append({
                'img_file': img_file,
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    ...
    local_container['pred_classes'] = prediction.get_field('labels').long().detach().cpu().numpy()     # (#pred_objs, )
    local_container['obj_scores'] = prediction.get_field('scores').detach().cpu().numpy()              # (#pred_objs, )
